#!/bin/bash
# ==========================================
# Aldow Premium MOTD Installer (v3 PRO)
# FULL CLEAN + ONLY CUSTOM MOTD
# ==========================================

set -e

echo "🔧 Installing Aldow Premium MOTD..."
...
# Remove previous Aldow pam_exec entries if rerunning
sed -i '/00-aldow/d' /etc/pam.d/sshd
sed -i '/00-aldow/d' /etc/pam.d/login

# Add ONLY our MOTD
echo "session optional pam_exec.so stdout /etc/update-motd.d/00-aldow" >> /etc/pam.d/sshd
echo "session optional pam_exec.so stdout /etc/update-motd.d/00-aldow" >> /etc/pam.d/login

# ================================
# CREATE PREMIUM MOTD SCRIPT
# ================================
echo "✨ Creating Aldow MOTD..."

cat << 'EOF' > /etc/update-motd.d/00-aldow
#!/bin/bash
...
# ===== HEADER =====
echo -e "${GREEN}🚀 Welcome to Aldow Datacenter${RESET}"
...
chmod +x /etc/update-motd.d/00-aldow
...
echo ""
echo "✅ Aldow MOTD Installed (ONLY MODE ENABLED)"
